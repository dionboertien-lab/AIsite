...
import { berekenProfiel, VraagAntwoord, WaardeNaam } from '../../lib/algoritme'

type Props = {
  navigation: NativeStackNavigationProp<OnboardingStackParams, 'ProfielReveal'>
  route: RouteProp<OnboardingStackParams & { ProfielReveal: { antwoorden: VraagAntwoord[] } }, 'ProfielReveal'>
}
...
export default function ProfielRevealScreen({ navigation, route }: Props) {
  ...
  const [top5, setTop5] = useState<WaardeNaam[]>([])
  const [zichtbaar, setZichtbaar] = useState(0)

  useEffect(() => {
    const antwoorden: VraagAntwoord[] = (route.params as any)?.antwoorden ?? []
    const resultaat = berekenProfiel(antwoorden)
    setTop5(resultaat.top5)

    // Animeer de reveal: één waarde per 400ms
    setTimeout(() => {
      setLaden(false)
      resultaat.top5.forEach((_, i) => {
      ...
      <View style={stijlen.inhoud}>
          ...
          {top5.map((waarde, index) => (
            ...
            >
              ...
              <View style={stijlen.waardeInfo}>
                ...
                <Text style={stijlen.waardeOmschrijving}>
                  {WAARDE_OMSCHRIJVING[waarde]}
                </Text>
              </View>
            </View>
          ))}
        ...
        <View style={stijlen.onderaan}>
          ...
          <TouchableOpacity
            style={stijlen.knop}
            onPress={() =>
              navigation.navigate('GapMeting', { antwoorden: (route.params as any)?.antwoorden, top5 } as never)
            }
          >
